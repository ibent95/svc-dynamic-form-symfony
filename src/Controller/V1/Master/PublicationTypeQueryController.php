...
class PublicationTypeQueryController extends AbstractController
{
    ...
    public function index(): JsonResponse
    {
        ...
        $this->responseData['message'] = 'Success to access the publication types API!';
    ...
    public function getAll(ManagerRegistry $doctrine, CommonService $common): JsonResponse
    {
        ...
        try {
            ...
            $publicationTypes                = $entityManager->getRepository(PublicationType::class)->
                findBy($publicationTypesParams);

            // Response data
            $this->responseData['data']     = $publicationTypes;
            $this->responseData['info']     = 'success';
            $this->responseData['message']  = 'Success to get publication types data!';
            $this->responseStatusCode       = 200;

            $this->logger->info('Get publication types data: ' . json_encode($this->responseData['data']));
        } catch (\Exception $e) {
            $this->responseData['message']  = 'Error on get publication types data!';
            $this->responseStatusCode       = 400;
            $this->logger->error(
                'Get publication types data exception log: ' . $e->getMessage() . ', line: ' . $e->getLine(),
    ...
    public function getMasterDataAll(ManagerRegistry $doctrine, CommonService $common): JsonResponse
    {
        ...
        try {
            ...
            $this->logger->info('Get master data of publication types: ' . json_encode($this->responseData['data']));
        } catch (\Exception $e) {
            $this->responseData['message']  = 'Error on get master data of publication types!';
            $this->responseStatusCode       = 400;
            $this->logger->error(
                'Get master data of publication types exception log: ' . $e->getMessage() . ', line: ' . $e->getLine(),
                [$e->getFile(), 'trace => ', $e->getTrace()]
            );
        }
